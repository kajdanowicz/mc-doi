...
from model.multi_contagion_models import MultiContagionDynamicLinearThresholdModelWithForgetting as MCDOIwf
import numpy as np
...
directory = '/nfs/maciej/mcdoi/cdlt-with-forgetting/'
...
def predict(path, num_predictions, predicted):
    if path not in predicted:
        ...
        new_path = path.split('/')
        new_path[4] = 'cdlt-with-forgetting'
