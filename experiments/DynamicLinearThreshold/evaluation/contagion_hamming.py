...
def evaluate(path, iter_length, model):
    ...
    for i in range(1,min(7,33-history)+1):
        open(new_path + '/contagion_hamming_' + str(i - 1), 'w', encoding='utf-8').close()
        for contagion_id in range(d.num_contagions):
            with open(new_path + '/contagion_hamming_' + str(i - 1), 'a', encoding='utf-8') as file:
                file.write(rev_contagion_dict[contagion_id] + ',' + str(hamming(indicators[i-1][:,contagion_id],results[i-1][:,contagion_id])) + '\n')
        with open(directory + 'frequencies/contagion_hamming_' + str(batch_size), 'a', encoding='utf-8') as file:
            file.write(new_path + '/contagion_hamming_' + str(i - 1) + '\n')
