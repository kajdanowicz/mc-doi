...
evaluated = set()
with open(directory + 'frequencies/contagion_hamming', 'r', encoding='utf-8') as file:
    e = file.readlines()
...
def evaluate(path, iter_length):
    ...
    org_path_dataset_history = path.split('/')
    org_path_dataset_history[4] = 'louvain'
    org_path_dataset_history = '/' + os.path.join(*org_path_dataset_history)
    with open(os.path.dirname(org_path_dataset_history)+'/edges', 'r', encoding='utf-8') as f:
    ...
    with open(os.path.dirname(org_path_dataset_history)+'/event_log', 'r', encoding='utf-8') as f:
    ...
    with open(org_path_dataset_history +'/data_obj.pickle', 'rb') as f:
        d=pickle.load(f)

    with open(org_path_dataset_history+'/contagion_dict.pickle', 'rb') as f:
    ...
    for i in range(1,min(7,33-history)+1):
        open(path + '/contagion_hamming_' + str(i - 1), 'w', encoding='utf-8').close()
        for contagion_id in range(d.num_contagions):
            with open(path + '/contagion_hamming_' + str(i - 1), 'a', encoding='utf-8') as file:
                file.write(rev_contagion_dict[contagion_id] + ',' + str(hamming(indicators[i-1][:,contagion_id],results[i-1][:,contagion_id])) + '\n')
        with open(directory + 'frequencies/contagion_hamming', 'a', encoding='utf-8') as file:
            file.write(path + '/contagion_hamming_' + str(i - 1) + '\n')
