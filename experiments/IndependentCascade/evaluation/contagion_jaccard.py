...
evaluated = set()
with open(directory + 'frequencies/contagion_jaccard', 'r', encoding='utf-8') as file:
    e = file.readlines()
...
def evaluate(path, iter_length):
    ...
    for i in range(1,min(7,33-history)+1):
        open(path + '/contagion_jaccard_' + str(i - 1), 'w', encoding='utf-8').close()
        for contagion_id in range(d.num_contagions):
            ...
            if union.size==0:
                with open(path + '/contagion_jaccard_' + str(i - 1), 'a', encoding='utf-8') as file:
        ...
        with open(directory + 'frequencies/contagion_jaccard', 'a', encoding='utf-8') as file:
            file.write(path + '/contagion_jaccard_' + str(i - 1) + '\n')
