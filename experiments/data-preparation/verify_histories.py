...
for dataset in tqdm(next(os.walk(directory))[1]):
    ...
    edges = pd.read_csv(set + '/edges')
    edges.columns = ['user1','user2']
    for history in tqdm(range(1,31)):
        file_name = set + '/history_' + str(history)
        event_log = pd.read_csv(file_name + '/event_log')
        event_log.columns = ['ts', 'user', 'contagion']
        if not set(event_log['user']).issubset(edges['user_1'].append(edges['user_2'])):
            print(file_name)
