...
for batch_size in [3600, 43200, 86400, 604800]:
    with open(directory + 'frequencies/contagion_fractions_'+str(batch_size), 'r', encoding='utf-8') as file:
        e = file.readlines()
...
def evaluate(path, iter_length):
    ...
    for i in range(1,min(7,33-history)+1):
        ...
        with open(directory + 'frequencies/contagion_fractions_' + str(batch_size), 'a', encoding='utf-8') as file:
