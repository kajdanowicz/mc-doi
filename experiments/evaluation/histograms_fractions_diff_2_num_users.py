...
directory = '/nfs/maciej/mcdoi/no-negative-exclusion/'
...
def histogram(list_of_directories,batch_size):
    ...
    for i in range(7):
        ...
        plt.savefig(directory+'histograms/fractions_diff_' + str(num_users)+'/batch_'+str(batch_size)+'_pred_'+str(i)+'.png', dpi=72)
        plt.close('all')
        plt.figure()
        ax = plt.subplot(111)
        ...
        ax.set_xlim(0,1)
        plt.savefig(directory+'histograms/fractions_diff_' + str(num_users)+'/scatter_batch_'+str(batch_size)+'_pred_'+str(i)+'.png', dpi=72)
        plt.close('all')
