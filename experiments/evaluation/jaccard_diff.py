...
for batch_size in [3600, 43200, 86400, 604800]:
    with open(directory + 'frequencies/jaccard_diff_'+str(batch_size), 'r', encoding='utf-8') as file:
        e = file.readlines()
...
def evaluate(path, iter_length):
    ...
    for i in range(1,min(7,33-history)+1):
        open(path + '/jaccard_diff_' + str(i - 1), 'w', encoding='utf-8').close()
        ...
        for user in range(d.num_users):
            ...
            if union.size==0:
                with open(path + '/jaccard_diff_' + str(i - 1), 'a', encoding='utf-8') as file:
                    file.write(str(user) + ',' + str(1) + '\n')
            else:
                with open(path + '/jaccard_diff_' + str(i - 1), 'a', encoding='utf-8') as file:
                    file.write(str(user) + ',' + str(intersection.size/union.size) + '\n')
        with open(directory + 'frequencies/jaccard_diff_' + str(batch_size), 'a', encoding='utf-8') as file:
            file.write(path + '/jaccard_diff_' + str(i - 1) + '\n')
