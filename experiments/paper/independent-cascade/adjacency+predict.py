...
directory = '/nfs/maciej/mcdoi/paper/independent-cascade/'

with open(directory+'estimated_adjacency', 'r', encoding='utf-8') as file:
...
#         estimate_adjacency_and_predict(dataset+'/history_'+str(history_length), 7, estimated)


def estimate_adjacency_and_predict(path_dataset_history, num_predictions, estimated, predicted):
    if path_dataset_history not in predicted:
        ...
        new_path_dataset_history[4] = 'paper/independent-cascade'
        ...
        if path_dataset_history not in estimated:
            ...
            with open(directory+'estimated_adjacency', 'a+', encoding='utf-8') as handle:
                handle.write(path_dataset_history + '\n')
            with open(directory+'predicted', 'a+', encoding='utf-8') as handle:
                handle.write(path_dataset_history + '\n')
        else:
            with open(new_path_dataset_history + '/adjacency.pickle', 'rb') as file:
                a = pickle.load(file)
...
if __name__ == '__main__':
    aprun(bar='txt')(delayed(estimate_adjacency_and_predict)(dataset, 7, estimated, predicted) for dataset in sets_to_estimate)
