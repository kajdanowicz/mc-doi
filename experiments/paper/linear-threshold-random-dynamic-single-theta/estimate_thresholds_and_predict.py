...
from model.single_contagion_models import SingleContagionLinearThresholdModel_randomDynamic14all as MCDOI
import numpy as np

##### Experiments parameters ######
model = 'linear-threshold-random-dynamic-single-theta'
...
if os.path.isfile(directory+'estimated_thresholds'):
    with open(directory+'estimated_thresholds', 'r', encoding='utf-8') as file:
        estimated = file.readlines()
    estimated_thresholds = [x.strip() for x in estimated]
else:
    open(directory + 'estimated_thresholds', 'w+', encoding='utf-8').close()
...
def estimate_t_and_predict(path_dataset_history, batch_type, batch_sizes, num_predictions, estimated, predicted):
    flag = False
    ...
    if flag:
        ...
        for batch_size in batch_sizes:
            if path_dataset_history+ '/' + batch_type + '/size_' + str(batch_size) not in estimated:
                ...
                with open(new_path_dataset_history + '/' + batch_type + '/size_' + str(batch_size) + '/threshold.pickle', 'rb') as file:
                    t = pickle.load(file)
                m = MCDOI()
                m.assign_adjacency_matrix(a)
...
if __name__ == '__main__':
    aprun(bar='txt')(delayed(estimate_t_and_predict)(dat, 'time', batch_sizes, number_prediction_intervals, estimated_thresholds, predicted) for dat in sets_to_estimate)
