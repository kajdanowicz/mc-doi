...
tmp =[]
for s in sets_to_estimate:
    tmp.append(s+'/time' + '/size_604800')

sets_to_estimate = tmp
...
aprun = ParallelExecutor(n_jobs=4)
...
estimated.append('/nfs/maciej/mcdoi/louvain/louvain_48_2234/history_3/time/size_604800')
...
def estimate_t_and_predict(path_dataset_history, batch_type, batch_sizes, num_predictions, estimated):
    path_dataset_h = os.path.dirname(os.path.dirname(path_dataset_history))
    edges = pd.read_csv(os.path.dirname(path_dataset_h) + '/edges', header = None)
    event_log = pd.read_csv(path_dataset_h + '/event_log', header=None)
    with open(path_dataset_h + '/contagion.pickle', 'rb') as file:
        cc = pickle.load(file)
    with open(path_dataset_h + '/adjacency.pickle', 'rb') as file:
        a = pickle.load(file)
    if os.path.isfile(path_dataset_h + '/data_obj.pickle'):
        with open(path_dataset_h + '/data_obj.pickle', 'rb') as f:
            d = pickle.load(f)
    else:
        d = Data()
        d.load_data_data_frame(event_log, edges)
        with open(path_dataset_h + '/data_obj.pickle', 'wb') as f:
            pickle.dump(d, f)
    m = MCDOI()
    m.assign_contagions_correlation_matrix(cc)
    m.assign_adjacency_matrix(a)
    m.fit_only_thresholds_states(d, batch_type = batch_type, batch_size = 604800)
    new_path_dataset_history = path_dataset_h.split('/')
    new_path_dataset_history[4] = 'correlated-linear-threshold'
    new_path_dataset_history = '/'+os.path.join(*new_path_dataset_history)
    file_name = new_path_dataset_history + '/' + batch_type + '/size_' + str(604800) + '/threshold.pickle'
    os.makedirs(os.path.dirname(file_name), exist_ok=True)
    with open(file_name, 'wb') as threshold_file:
        pickle.dump(m.thresholds.matrix, threshold_file)
    result = m.predict(num_predictions)
    save_results(result, new_path_dataset_history + '/' + batch_type + '/size_' + str(604800), num_predictions)
...
if __name__ == '__main__':
    aprun(bar='txt')(delayed(estimate_t_and_predict)(dat, 'time', batch_sizes, 7, estimated) for dat in diff(sets_to_estimate,estimated))
