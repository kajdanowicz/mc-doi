...
class ThresholdBase(BaseParameter):
    ...
    def estimate(self, data: Data, **kwargs):
        ...
        kwargs
        """
        self.matrix = np.repeat(np.random.random(data.num_users)[np.newaxis].T, data.num_contagions, axis=1)
    ...
    def reestimate(self):
        ...
        num_contagions = self.matrix.shape[1]
        self.matrix = np.repeat(np.random.random(num_users)[np.newaxis].T, num_contagions, axis=1)
