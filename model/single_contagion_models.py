...
from model.parameters_independent_cascade import Adjacency as Adjacency_IndependentCascade
...
class SingleContagionIndependentCascade(MultiContagionDynamicLinearThresholdModel):

    def __init__(self):

        # super(SingleContagionDynamicLinearThresholdModel, self).__init__()
        self.adjacency = Adjacency_IndependentCascade()
    ...
    def __single_iteration(self) -> SingleIterResult:
        # TODO SingleIterResult -> new special class
        self.newly_activated_future_ = ([], [])
        for counter, user in enumerate(self.newly_activated_[0]):
            random_draw = np.random.rand(self.state_matrix_.num_users)
            random_diffusion = np.greater(self.adjacency.matrix[user,:].tolist(),random_draw)
            activation_candidates = np.where(random_diffusion[0])
            for candidate in activation_candidates[0]:
                if not self.state_matrix_.matrix[candidate,self.newly_activated_[1][counter]]:
                    self.newly_activated_future_[0].append(candidate)
